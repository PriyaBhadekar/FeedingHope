...
if(isset($_GET['status'])){
	...
	if($_GET['status'] == "Approved"){
		$sqlchild_adoption = "SELECT * FROM child_adoption WHERE child_adoption_id ='" . $_GET['editid'] . "'";
		$qsqlchild_adoption = mysqli_query($con,$sqlchild_adoption);
		$rschild_adoption = mysqli_fetch_array($qsqlchild_adoption);

		$sqlmember = "SELECT * FROM member WHERE member_id ='" . $rschild_adoption['member_id'] . "' AND status='Adopted'";
		$qsqlmember = mysqli_query($con,$sqlmember);
		$rsmember = mysqli_fetch_array($qsqlmember);
		$adaptno = mysqli_num_rows($qsqlmember);
		if(mysqli_num_rows($qsqlmember) == 1){
			echo "<script>alert('This child already adopted...');</script>";
...
<?php
	...
	while($rs = mysqli_fetch_array($qsql))
	{
		...
		$sqlmember = "SELECT member.*,member_type.member_type FROM member LEFT JOIN member_type ON member.member_type_id=member_type.member_type_id WHERE member.member_type_id='1' AND member.member_id='$rs[member_id]'";
		$qsqlmember = mysqli_query($con,$sqlmember);
		$rsmember = mysqli_fetch_array($qsqlmember);
		if($rsmember['image'] == "")
		{
